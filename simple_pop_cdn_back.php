...
class SimplePOPCDN
{
     ...
     * @param int $cache_expire Amount of time in seconds cache is valid for. 2628000 = 1 month , 7776000 = 3 months
     * @param int $min_valid_file_size 最小的有效文件大小 Byte
     */
    function __construct($origin = null, $cache_path = null, $fix_request = null, $cache_expire = 7776000,  $min_valid_file_size = 1024 )
    {
        $this->origin = $origin;
        ...
        $this->min_valid_file_size = $min_valid_file_size;
    ...
    private function setup($accepted = array())
    {
        ...
        // 加密文件名
        //$this->cache_name = basename($this->request);
        // set cache name hash from request
        //$this->cache_name = hash('sha256', $this->request);

        //保持原有目录和文件名称
        $this->cache_name = $this->request_info['dirname'] . '/' . $this->request_info['filename'];

        //如果有存在ver版本变量
        if(isset($_REQUEST['ver'])){
            //添加版本号到文件末端
            $this->cache_name .= '_'.$_REQUEST['ver'];
        }
		...
		//var_dump( $this->cache_name);
		//var_dump($this->cache_full);
		//exit();
    }
    ...
    private function handle()
    {
        ...
        if (file_exists($this->cache_full))
        {
            ...
            //如果文件还未过期
            if ($this->modified + $this->cache_expire > time())
            {
                $is_valid_file = true;
            }

            //如果小于 最小有效缓存图片
            if (filesize($this->cache_full) < $this->min_valid_file_size)
            {
                //删除文件 重新获取
                unlink($this->cache_full);
                $is_valid_file = false;
            }
        }
        ...
        if ($is_valid_file)
        {
            ...
            else
            {
                ...
                // Stream file
                set_time_limit(0);
                //$h = gzopen($this->cache_full, 'r');
                //while ($line = gzgets($h, 4096)) {
                //   echo $line;
                //}
                //gzclose($h);
                $filesize =  readfile($this->cache_full);
                //exit();
        ...
        else
        {
            ...
            //var_dump($this->origin . $this->request);
            ...
            {
                //如果对应文件夹目录不存在
                $dirname = dirname($this->cache_full);
                if (!is_dir($dirname))
                {
                    //递归创建需要的文件夹
                ...
                if (file_exists($this->cache_full))
                {
                    ...
                    ////如果小于1kb
                    //if(filesize($this->cache_full) < 1024){
                    //    unlink($this->cache_full);
                    //    $this->error($_SERVER['SERVER_PROTOCOL'] . ' 500 Can not get file from origin server');
                    //}
